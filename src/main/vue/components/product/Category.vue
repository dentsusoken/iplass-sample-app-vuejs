...
<div class="col-sm-12 col-md-9">
    <div class="row">
        <div class="col-12">
            ...
            <nav class="breadcrumb all-breadcrumb">
                ...
                <span class="breadcrumb-item active" :id="categoryId">{{categoryNameLocale}}</span>
            </nav>
        </div>
    </div>
    <div class="row d-none d-md-block">
        <div class="col-12 clearfix">
            ...
            <div class="dropdown float-end">
                <button id="categoryList" class="btn btn-outline-dark dropdown-toggle" type="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    {{ selectedCategory.name }}
                </button>
                <div class="dropdown-menu" aria-labelledby="categoryList">
                    <router-link class="dropdown-item" :to="{name: 'category', query: {categoryId: 'all'}}">
                        {{$t("samples.ec01.product.category.all")}}
                    </router-link>
                    <div class="dropdown-divider"></div>
                    <router-link v-for="c in categoryList" :key="c.oid" class="dropdown-item" :to="{name: 'category', query: {categoryId: c.oid}}">
                        {{c.name}}
                    </router-link>
                </div>
            </div>
    ...
    <div class="row">
        <div class="col-12 all-pagination-list" >
            <!-- ページング -->
            <pagination v-bind="{pagination: pagination, paginationUrl: paginationUrl}" :id="page"></pagination>
        </div>
    </div>
</div>
</template>
